...
/** Orchestration engines recognized by the loader (mirrors GraphOrchestration). */
const KNOWN_ORCHESTRATIONS = ["graph_v2"] as const satisfies readonly GraphRoleConfig["orchestration"][];

/** @internal Narrowing guard for a recognized orchestration engine string. */
function isKnownOrchestration(value: string): value is GraphRoleConfig["orchestration"] & string {
  return (KNOWN_ORCHESTRATIONS as readonly string[]).includes(value);
}

/**
 * Parse the role-level `graph:` block.
 *
 * The `graph:` key is recognized but not yet wired into graph resolution,
 * so the loader must never drop it silently. A valid declaration is surfaced
 * on {@link RoleConfig.graph} AND logged with a warning naming the role and
 * the key; malformed declarations are warned about and omitted.
 */
function parseGraphConfig(
  raw: Record<string, unknown> | null | undefined,
  roleId: string,
): GraphRoleConfig | undefined {
  if (raw == null || typeof raw !== "object") return undefined;

  const orchestration = (raw as Record<string, unknown>).orchestration;
  if (typeof orchestration !== "string" || orchestration.trim() === "") {
    log.warn(
      `Role "${roleId}" declares "graph:" without a valid orchestration selector — key ignored`
    );
    return undefined;
  }

  const engine = orchestration.trim();
  if (!isKnownOrchestration(engine)) {
    log.warn(
      `Role "${roleId}" declares unknown graph.orchestration "${engine}" — key ignored`
    );
    return undefined;
  }

  log.warn(
    `Role "${roleId}" declares "graph.orchestration: ${engine}" — key recognized but not yet wired into graph resolution (no runtime effect)`
  );
...
function buildRoleConfig(
  ...
  graphConfig: GraphRoleConfig | undefined,
): RoleConfig {
  return {
...
async function loadOneRole(
  ...
  const graphConfig = parseGraphConfig(
    resolved.graph as Record<string, unknown> | null | undefined,
    roleId,
  );
  ...
  const config = buildRoleConfig(resolved, prompt, dispatchConfig, graphConfig);
