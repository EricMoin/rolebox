...
export interface DispatchParentContext {
  /**
   * Graph-scoped request/budget key. For a graph-level dispatch this is the
   * graph ID (see {@link graphParentContext}); the dispatch subsystem treats
   * the parent session ID as the **request** scope, seeding `requestUsage`
   * and `getBudgetTracker().getRequestUsage(...)` keyed off it. This is a
   * budget/request scope key — NOT necessarily a live platform session handle.
   */
  sessionID: string;
  /**
   * REAL live parent session that owns this dispatch — the platform session
   * handle required by adapters (e.g. dsh) that must post notifications or
   * resume a run against a live parent. Distinct from {@link sessionID}, which
   * is the graph-scoped budget/request key. Omitted by
   * {@link graphParentContext} when the caller supplies no live parent, so
   * opencode/Pi contexts stay byte-identical to before this field existed.
   */
...
export interface GraphParentOptions {
  ...
  directory: string;
  /**
   * REAL live parent session that owns the graph run. Copied onto the returned
   * context's {@link DispatchParentContext.parentSessionId} only when supplied.
   * Platform adapters that require a live parent handle (dsh) read it in place
   * of the graph-scoped {@link DispatchParentContext.sessionID}. Omit it for
   * opencode/Pi, whose contexts are unchanged.
   */
  parentSessionId?: string;
}
 ...
 *
 * When `opts.parentSessionId` is supplied it is copied onto
 * {@link DispatchParentContext.parentSessionId} — the REAL live parent session
 * that platform adapters requiring a live handle (dsh) use instead of the
 * graph-scoped `sessionID`. The key is omitted entirely when absent so
 * opencode/Pi contexts are byte-identical to the pre-field shape.
 */
export function graphParentContext(opts: GraphParentOptions): DispatchParentContext {
  return {
    sessionID: opts.graphId,
    // Real live parent handle (dsh); omitted when absent so opencode/Pi
    // contexts carry no extra key.
