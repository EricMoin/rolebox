...
export function createLoopStartTool(
  ...
  return defineTool({
    ...
    async execute(args, ctx) {
      ...
      // Validate the prompt before it reaches the coordinator. Direct
      // execute() calls bypass zod parsing, and register() fingerprints the
      // prompt via prompt.replace(...) — a missing/non-string prompt would
      // otherwise surface as a raw TypeError.
      if (typeof prompt !== "string" || prompt.trim().length === 0) {
        return 'Invalid prompt: "prompt" is required and must be a non-empty string.';
      }

      // Apply the schema defaults explicitly — direct execute() calls bypass
      // zod parsing, so undefined args would otherwise reach the coordinator.
      const iterations = args.iterations ?? 5;
      const mode = (args.mode ?? "inherit") as LoopMode;
