...
export function createSessionForkTool(client: ISessionClient) {
  return defineTool({
    ...
    async execute(input, context) {
      ...
      if (!forked) {
        // The session's existence was already proven by client.get() above, so
        // never blame a missing session here. Only a supplied message_id can
        // be the refused fork point.
        return input.message_id
          ? `Failed to fork session "${session.title}" (${input.session_id}) at message "${input.message_id}": the session exists, but the fork was refused — the message ID may be invalid.`
          : `Failed to fork session "${session.title}" (${input.session_id}): the session exists, but the fork was refused.`;
      }
