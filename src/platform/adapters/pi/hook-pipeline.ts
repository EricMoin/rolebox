 ...
 *   - `roleFunctionsMap` / `roleMap` = derived from the
 *     resolved roles (roleFunctionsMap is the module-scope map shared with
 *     pi-extension.ts)
 ...
 *     opencode-only)
 *
 * The persistent stores (functionRuntime, sessionSignalLedger) are pointed at
 * `dir` and recovered exactly as hook-service.ts:59-66 does, so function and
 * signal state survives restarts on the Pi platform.
 *
...
export interface PiHookPipelineOptions {
  ...
  /** Resolved roles (source of roleFunctionsMap/roleMap and custom hooks). */
