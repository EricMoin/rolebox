...
export class DshHookProvider implements IHookProvider {
  ...
  private wire(): void {
    ...
    // cordis waterfall events invoke each listener as (…payloadArgs, next) and
    // the listener MUST return the chain result; a bare `undefined` clobbers
    ...
    for (const { dshEvent, kind } of mappings) {
      const listener = (...args: unknown[]): unknown => {
        const isWaterfall = waterfallEvents.has(dshEvent);
        const maybeNext = isWaterfall ? args[args.length - 1] : undefined;
        const next =
          typeof maybeNext === "function" ? (maybeNext as () => unknown) : undefined;
        // Waterfall listeners receive a trailing `next`; strip it before payload
        // normalization, then always chain so the waterfall keeps its default
        // decision.
        const payloadArgs = next ? args.slice(0, -1) : args;
        const proceed = (): unknown => (next ? next() : undefined);

        const payload = this.toPayload(dshEvent, payloadArgs);
        ...
        let result: unknown;
        try {
          result = this.handlers[kind](payload);
        } catch (err) {
          // Observer hooks never gate: log and keep the waterfall alive.
          this._log.warn("dsh hook handler threw", {
            kind,
            dshEvent,
            error: err instanceof Error ? err.message : String(err),
          });
        ...
        // Handlers may return a promise; never throw into cordis, and chain the
        // waterfall only after the observer settles.
        if (result && typeof (result as Promise<unknown>).catch === "function") {
          return (result as Promise<unknown>).then(proceed, (err) => {
   ...
   *
   * Event-aware. `session/event` delivers `(session, event)` in rc.6
   * (`dsh-session/lib/types/index.d.ts:66`), so the session id is read from
   * arg0 and the sub-type/`seq`/`time`/`data` from arg1. Tool events keep the
   * tolerant `(payload)` / `(name, payload)` handling, and the outcome
   * argument of `tools/post-execute` / `tools/result` is surfaced as
   * `payload.result`. Common field aliases are normalized (`name` → `tool`,
   * `callId` → `callID`, `sessionId` → `sessionID`).
   */
  private toPayload(dshEvent: string, args: unknown[]): DshHookPayload {
    ...
    if (dshEvent === "session/event") {
      this.mergeSessionEvent(payload, args);
    } else {
      this.mergeToolArgs(payload, args);
      this.mergeToolResult(payload, dshEvent, args);
    }
  ...
  /**
   * Normalize the rc.6 `session/event(session, event)` args. The session id
   * comes from arg0 (`Session.id`); the sub-type (`type`), `seq`, `time`, and
   * `data` come from arg1 (`SessionEvent`). A `Session` carries no `type`, so
   * merging arg0 as the event would leave `sessionEventType` undefined and
   * silently kill every `chat-message` hook.
   */
  private mergeSessionEvent(payload: DshHookPayload, args: unknown[]): void {
    ...
    if (typeof event.type === "string") {
      payload.sessionEventType = event.type;
    }
  ...
  /**
   * Tolerant tool-event arg handling: `(payload)` where payload is an object,
   * and `(name, payload)` where the tool name leads.
   */
  private mergeToolArgs(payload: DshHookPayload, args: unknown[]): void {
  ...
  /**
   * `tools/post-execute(exec, result, next)` and `tools/result(exec, result)`
   * carry the outcome as the SECOND argument; surface it under
   * `payload.result` so `tool-after` hooks can observe it. `next` is already
   * stripped by the listener for waterfall events.
   */
  private mergeToolResult(
    ...
    if (dshEvent !== "tools/post-execute" && dshEvent !== "tools/result") {
      return;
    }
    const result = args[1];
    if (result !== undefined) {
