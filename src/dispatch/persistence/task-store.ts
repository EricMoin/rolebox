...
import { acquireStateLock } from "../concurrency/state-lock.ts";
import type { DispatchTask, TaskEventState } from "../types.ts";
import type {
  SerializedDispatchTask,
  SerializedTaskEventState,
  SerializedTaskProgress,
  DispatchStateFile,
} from "./task-serialization.ts";

export type { SerializedDispatchTask } from "./task-serialization.ts";
  ...
  load(): { tasks: Map<string, DispatchTask>; outbox: string[]; eventState: Map<string, TaskEventState> } | null {
    ...
    for (const st of tasks) {
      map.set(st.id, {
        ...
        suppressCompletionNotification: st.suppressCompletionNotification,
        status: st.status,
        agent: st.agent,
        prompt: st.prompt,
  ...
  private serialize(
    ...
    for (const task of tasks.values()) {
      const s: SerializedDispatchTask = {
        ...
        suppressCompletionNotification: task.suppressCompletionNotification,
        status: task.status,
        agent: task.agent,
        prompt: task.prompt,
