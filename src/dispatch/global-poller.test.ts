...
import { GlobalPoller, type GlobalPollerDeps } from "./global-poller";
import { SessionMonitor } from "./session-monitor";
import type { SessionMessageSnapshot, TaskPollState } from "./types";
import { MIN_POLL_INTERVAL_MS, MAX_POLL_INTERVAL_MS, DEFAULT_MAX_CONCURRENT } from "./config";
...
function makePoller(m: Mocks, configOverrides?: Partial<{ maxConcurrent: number }>) {
  return new GlobalPoller(m.client, {
    ...
    completionDetector: m.completionDetector as unknown as typeof import("./completion-detector").detectCompletion,
