...
/**
 * Pick a sync target (host platform) from the platform registry.
 *
 * Choices are registry-driven (`PLATFORM_REGISTRY`) — never a hardcoded list,
 * so a newly registered harness appears automatically. When `role` is known,
 * targets where that role is already synced are surfaced first, so the user is
 * not led to a target where the caller would immediately fail with "no roles
 * synced"; if no target has the role (or no role was given), every valid
 * target is listed so the flow still works before the first sync.
 *
 * Returns the chosen target id, or null when the user cancels.
 */
export async function pickTarget(
  ...
  const syncedIds = new Set(
    role
      ? PLATFORM_REGISTRY.filter((p) => existsSync(join(getSyncTarget(p.id), role))).map((p) => p.id)
      : [],
  );
  const candidates =
    syncedIds.size > 0
      ? PLATFORM_REGISTRY.filter((p) => syncedIds.has(p.id))
      : [...PLATFORM_REGISTRY];
  ...
  const picked = await prompts.select({
    ...
    options: candidates.map((p) => ({
      value: p.id,
      label: p.label,
      hint: role ? (syncedIds.has(p.id) ? "synced" : "not synced") : undefined,
    })),
    maxItems: PLATFORM_REGISTRY.length,
  });
