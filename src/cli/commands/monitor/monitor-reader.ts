...
import { readLoopSnapshots, readGraphEvents } from "./monitor-reader-graph.ts";
...
export { readLoopSnapshots, readGraphEvents } from "./monitor-reader-graph.ts";
...
export function readMonitorSnapshot(projectDir: string, tailChars = 0): MonitorSnapshot {
  ...
  for (const af of activeFunctions) {
    af.agentId = sessionAgentMap.get(af.sessionId) ?? null;
  }
  ...
  // `dispatchSessionId`, not the graphId. The graph's own `phase` already
  // conveys liveness, so a persisted engine graph is surfaced even when no
  // running dispatch task sessionId equals its graphId.
  const engineGraphs = readEngineGraphs(stateDir);
  const graphEvents = readGraphEvents(stateDir);
  // Cross-filter loops by live sessions
