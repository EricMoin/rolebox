...
import { DEFAULT_FUNCTIONS } from "../../../constants.ts";
 ...
 *
 * For the `subagents` template a `subagents/{name}/`
...
export async function scaffoldRole(
  ...
  // Sub-agent scaffolding for the subagents template
  if (templateType === "subagents") {
    // Extra READMEs only present in the subagents template
