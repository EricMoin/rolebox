...
export interface GraphDeclaration {
  ...
  name: string;
  /**
   * Optional topology shorthand (e.g. "pipeline", "review-loop", "star").
   * Ignored by the graph engine; retained as declaration metadata for
   * round-trip parsing and human-readable inspection. Absent for native graphs.
   */
  template?: GraphTemplate;
  /**
   * Optional effective max-iteration cap (explicit value clamped to ≥0, else 3
   * on a cycle, else 0). Ignored by the engine; retained as declaration
   * metadata for round-trip parsing.
   */
...
export interface EdgeDeclaration {
  ...
  to: string;
  /**
   * Optional human-readable edge label (e.g. "loop", "exit", "handoff").
   * The engine ignores it; retained as declaration metadata. Absent for
   * unlabeled edges.
   */
  label?: string;
