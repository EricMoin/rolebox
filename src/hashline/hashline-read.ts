  ...
  return defineTool({
    ...
    args: {
      filePath: z
        .string()
        .describe(
          "Path to the file to read. Relative paths resolve against the session working directory.",
        ),
      offset: z
        .number()
        .int()
    ...
    async execute(input, context) {
      const filePath = resolveHashlinePath(input.filePath, context);
      try {
        const raw = await readFile(filePath, "utf-8");
        return formatReadOutput(raw, filePath, input.offset, input.limit);
      } catch (err: unknown) {
