...
import { resolveHashlinePath } from "./path-resolve.ts";
...
export function createHashlineEditTool(hooks: HashlineEditToolHooks = {}) {
  return defineTool({
        ...
        .array(
          z.object({
            filePath: z
              .string()
              .describe(
                "Path to the file to edit. Relative paths resolve against the session working directory.",
              ),
    ...
    async execute(input, context) {
      // Resolve every input path against the session workspace up front, so the
      // duplicate check, path locks, file read, pre-write recheck, and write all
      // operate on the same physical path. A relative path left unresolved would
      // be interpreted against the host process cwd by readFile and by
      // normalizeLockKey's path.resolve, diverging from the session workspace.
      const files = input.files.map((file) => ({
        ...file,
        filePath: resolveHashlinePath(file.filePath, context),
      }));
