...
/**
 * Separator-insensitive path comparison — the canonical way to test whether two
 * paths refer to the same location regardless of `\` vs `/`.
 *
 * This is a pure string → boolean function with **no platform branch**: it does
 * not consult `path.sep` or `process.platform`, so win32 semantics are fully
 * exercisable on POSIX (see tests/utils/paths.test.ts). Both operands are
 * normalized with `toPosixPath` and compared. Rationale for normalizing before
 * comparison matches src/resolver/skill-resolver.ts:57-63 — backslash-built
 * (win32 `join`) and forward-slash (fast-glob) paths must agree, and on POSIX
 * `toPosixPath` is a no-op so behavior is unchanged there.
 */
