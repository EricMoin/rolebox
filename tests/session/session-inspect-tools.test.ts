...
describe("createSessionForkTool", () => {
  ...
  it("fork refusal without message_id reports the session exists and does not mention message_id", async () => {
    const tool = createSessionForkTool(mockClient({
      sessionGet: () => Promise.resolve(makeSession({ id: "ses_1", title: "Existing" })),
      sessionFork: () => Promise.resolve(null),
    }));
    const result = (await tool.execute(args as never, defaultCtx)) as string;
    expect(result).toContain("Failed to fork session");
    ...
    expect(result).not.toContain("may not exist");
    expect(result.toLowerCase()).not.toContain("message");
  });

  it("fork refusal with message_id supplied mentions the message_id", async () => {
    const tool = createSessionForkTool(mockClient({
      sessionGet: () => Promise.resolve(makeSession({ id: "ses_1", title: "Existing" })),
      sessionFork: () => Promise.resolve(null),
    }));
    ...
    expect(result).toContain("Failed to fork session");
    expect(result.toLowerCase()).toContain("message");
    expect(result).toContain("msg_3");
  });
