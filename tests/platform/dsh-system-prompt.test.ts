 ...
 *   - register() is idempotent under re-registration: a second call on the
 *     same adapter disposes its prior entries first, so the duplicate-name-
 *     throwing registry never sees a duplicate and exactly one entry per
 *     contribution name survives
 ...
 * `text` providers) and returns disposers that record their invocation AND
 * remove the entry, making `dispose()` verifiable. Mirrors the real
 * `@deepseek-ai/dsh-system-prompt` rc.6 registry: `section(entry) -> disposer`,
 * `context(entry) -> disposer`, and a duplicate name in the same layer THROWS
 * (`prompt section "<name>" is already registered`) rather than replacing —
 * `@deepseek-ai/dsh-system-prompt/lib/index.js:142-144,188,198`.
 */
function createFakeRegistry() {
  ...
  const registry: DshSystemPromptRegistry = {
    section(entry: DshSystemPromptSection): () => void {
      if (sections.some((s) => s.name === entry.name)) {
        throw new Error(`prompt section "${entry.name}" is already registered`);
      }
      sections.push(entry);
      return () => {
    ...
    context(entry: DshSystemPromptContextEntry): () => void {
      if (contexts.some((c) => c.name === entry.name)) {
        throw new Error(`prompt context "${entry.name}" is already registered`);
      }
      contexts.push(entry);
      return () => {
        const i = contexts.indexOf(entry);
...
describe("DshSystemPromptAdapter.register", () => {
  ...
  it("register() twice on the same adapter is idempotent — no throw, one entry per name", async () => {
    const fixture = createFixture([makeAgent("tester")]);
    const { activeRole, adapter } = await fixture.build();
    const { registry, sections, contexts } = createFakeRegistry();

    adapter.register(registry);
    // The fake THROWS on a duplicate name (mirroring rc.6), so a naive
    // second register would throw — the adapter disposes its prior entries
    // first.
    ...
    // The re-registered section provider is still live (the second
    // registration replaced the first, not merely cleared it).
  ...
  it("the throwing fake proves the old replace-assumption is gone", async () => {
    const fixture = createFixture([makeAgent("tester")]);
    ...
    // A bare duplicate registration of the same name (no adapter disposal)
    // throws — the real rc.6 registry does NOT replace on a duplicate.
    expect(() =>
      registry.section({ name: "rolebox:role", order: 50, text: () => "" }),
    ).toThrow(/rolebox:role.*already registered/);
  });
});
