...
describe("DshEventBridge", () => {
  ...
  it("maps session/event sub-types (turn/end → session.idle) from the 2-arg (session, event) form", async () => {
    const ctx = createFakeCtx();
    const bridge = new DshEventBridge(ctx);
    ...
    // rc.6 emits (session, event): arg0 is the Session, arg1 the SessionEvent.
    ctx.emit(
      "session/event",
      { id: "s1" },
      { seq: 3, type: "turn/end", data: { stopReason: "completed" } },
    );
    await flush();
  ...
  it("regression: 2-arg session/event assistant/message yields message.created with rawType assistant/message", async () => {
    const ctx = createFakeCtx();
    const bridge = new DshEventBridge(ctx);
    ...
    // rc.6 (session, event): the sub-type must be read off arg1, and sessionID
    // off the Session (arg0). Reverting wire() to bind arg0 as the event makes
    // this map to session.updated/session/event and turns this test RED.
    ctx.emit(
      "session/event",
      { id: "s1" },
      {
    ...
    expect(received).toHaveLength(1);
    expect(received[0].type).toBe("message.created");
    expect(received[0].rawType).toBe("assistant/message");
    expect(received[0].properties.sessionID).toBe("s1");
    expect(received[0].properties.sourceEvent).toBe("session/event");
    bridge.dispose();
  });

  it("regression: legacy single-arg session/event no longer maps sub-types", async () => {
    const ctx = createFakeCtx();
    const bridge = new DshEventBridge(ctx);
    ...
    // The pre-fix fixture shape (event as the ONLY arg) is now arg0 = Session
    // and arg1 = undefined, so it must fall back to session.updated.
    ...
    expect(received[0].rawType).toBe("session/event");
    expect(received[0].type).not.toBe("message.created");
    bridge.dispose();
  });

  it("bridges session/flush from a single session arg (no flushed property)", async () => {
    const ctx = createFakeCtx();
    const bridge = new DshEventBridge(ctx);
    ...
    ctx.emit("session/flush", { id: "s1" });
    await flush();

    expect(received).toHaveLength(1);
    expect(received[0].type).toBe("session.updated");
    expect(received[0].rawType).toBe("session/flush");
    expect(received[0].properties.sessionID).toBe("s1");
    expect("flushed" in received[0].properties).toBe(false);
    bridge.dispose();
  });

  it("bridges tools/change with no args (no definition property)", async () => {
    const ctx = createFakeCtx();
    const bridge = new DshEventBridge(ctx);
    ...
    expect(received).toHaveLength(1);
    expect(received[0].type).toBe("session.updated");
    expect(received[0].rawType).toBe("tools/change");
    expect("definition" in received[0].properties).toBe(false);
    bridge.dispose();
  });
