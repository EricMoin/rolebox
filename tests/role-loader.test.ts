...
describe("discoverRoles", () => {
  ...
  describe("graph block parsing", () => {
    it("surfaces a valid graph: block and warns it is not yet wired (never silently dropped)", async () => {
      await writeRoleYaml(
        "graph-role",
        [
          "name: GraphRole",
          "description: Has graph config",
      ...
      expect(roles.size).toBe(1);
      const config = roles.get("graph-role")!;
      // Surfaced, not dropped
      expect(config.graph).toBeDefined();
      expect(config.graph!.orchestration).toBe("graph_v2");

      // And warned, naming the role and the key
      const warnings = capturedLogs as string[][];
      const graphWarnings = warnings.filter((c) =>
        typeof c[0] === "string" && c[0].includes("graph-role") && c[0].includes("graph"),
      );
      expect(graphWarnings.length).toBeGreaterThanOrEqual(1);
    });

    it("role without graph: block loads normally with graph undefined and no logs", async () => {
      await writeRoleYaml(
        "plain-graph",
      ...
      expect(roles.size).toBe(1);
      const config = roles.get("plain-graph")!;
      expect(config.graph).toBeUndefined();
      expect(capturedLogs.length).toBe(0);
    });

    it("warns and drops an unknown graph.orchestration, role still loads", async () => {
      await writeRoleYaml(
        "bad-graph",
        [
          "name: BadGraph",
          "description: Unknown orchestration",
          "prompt: I orchestrate oddly.",
          "graph:",
          "  orchestration: graph_v9",
        ].join("\n"),
      );
      ...
      expect(roles.size).toBe(1);
      const config = roles.get("bad-graph")!;
      expect(config.graph).toBeUndefined();

      const warnings = capturedLogs as string[][];
      const warningCount = warnings.filter((c) =>
        typeof c[0] === "string" && c[0].includes("bad-graph") && c[0].includes("graph"),
      ).length;
      expect(warningCount).toBeGreaterThanOrEqual(1);
    });

    it("warns on a graph: block with no orchestration selector, role still loads", async () => {
      await writeRoleYaml(
        "empty-graph",
        [
          "name: EmptyGraph",
          "description: Empty graph block",
          "prompt: I have an empty graph.",
          "graph: {}",
        ].join("\n"),
      );
      ...
      expect(roles.size).toBe(1);
      const config = roles.get("empty-graph")!;
      expect(config.graph).toBeUndefined();

      const warnings = capturedLogs as string[][];
      const warningCount = warnings.filter((c) =>
        typeof c[0] === "string" && c[0].includes("empty-graph") && c[0].includes("graph"),
      ).length;
      expect(warningCount).toBeGreaterThanOrEqual(1);
    });
  });
