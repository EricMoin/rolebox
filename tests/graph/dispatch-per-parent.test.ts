...
describe("graphParentContext", () => {
  it("returns a graph-scoped parent context (sessionID = graphId, graphScoped)", () => {
    ...
    // Additive `parentSessionId` field: omitted entirely when no live parent is
    // supplied, so opencode/Pi contexts stay byte-identical to the pre-field
    // shape.
    expect(ctx.parentSessionId).toBeUndefined();
  });

  it("carries a supplied live parentSessionId without disturbing graph scoping", () => {
    const LIVE_PARENT = "sess-live-parent";
    const ctx = graphParentContext({
      graphId: GRAPH_ID,
      directory: WORKDIR,
      parentSessionId: LIVE_PARENT,
    });

    expect(ctx.parentSessionId).toBe(LIVE_PARENT);
    // The additive field must not alter the graph-scoped request key or the
    // graph-scope marker.
    expect(ctx.sessionID).toBe(GRAPH_ID);
    expect(ctx.graphScoped).toBe(true);
  });
