...
describe("graph staleness → cascade → deadlock → retry chain (①→②→③→④)", () => {
  ...
  describe("(a) S2 dispatch-liveness gate", () => {
    it("a verifiably-live dispatch task survives past the 15-min staleness deadline; an absent-task sibling is wall-clock killed with 'dispatch task live=false'", async () => {
      ...
      // Each markRunning call stamps its own `startedAt = Date.now()`, so a
      // millisecond boundary between the two adjacent calls would leave C
      // starting 1ms after B. Ticking at b.startedAt + STALE_MS then fails to
      // reach C's deadline (now - c.startedAt === STALE_MS - 1), so C would
      // not time out. Pin both siblings to the same deterministic instant;
      // they conceptually start together.
      const t0 = Math.max(b.startedAt, c.startedAt);
      b.startedAt = t0;
      c.startedAt = t0;
      ...
      const timedOut = watcher.tick(state, t0 + STALE_MS);
