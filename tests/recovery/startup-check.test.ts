...
describe("StartupChecker", () => {
  ...
  it("recognizes activerole- as version-gated (valid kept, corrupt quarantined)", () => {
    const stateDir = tmpDir();
    // Valid v1 sidecar (the ActiveRoleStore envelope) → kept in place.
    writeStateFile(stateDir, "activerole-abc.json", {
      version: 1,
      sessions: [{ sessionId: "s", roleId: null, updatedAt: 1 }],
    });
    // Corrupt / out-of-range / versionless → quarantined.
    writeRawFile(stateDir, "activerole-bad-json.json", "not-json{{{");
    writeStateFile(stateDir, "activerole-future.json", { version: 100, sessions: [] });
    writeStateFile(stateDir, "activerole-versionless.json", { sessions: [] });

    const result = StartupChecker.checkAll(tmpDir(), stateDir);

    expect(result.healthy).toBe(false);
    expect(result.quarantined).toContain("activerole-bad-json.json");
    expect(result.quarantined).toContain("activerole-future.json");
    expect(result.quarantined).toContain("activerole-versionless.json");
    expect(result.quarantined).not.toContain("activerole-abc.json");
    expect(existsSync(join(stateDir, "activerole-abc.json"))).toBe(true);
  });

  it("treats a valid activerole- sidecar as healthy", () => {
    const stateDir = tmpDir();
    writeStateFile(stateDir, "activerole-abc.json", {
      version: 1,
      sessions: [{ sessionId: "s", roleId: "role-a", updatedAt: Date.now() }],
    });
