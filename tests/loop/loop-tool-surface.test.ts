...
describe("loop tool surface (opencode path)", () => {
  ...
  it("loop_start with empty args returns a prompt correction instead of throwing", async () => {
    const coordinator = new LoopCoordinator(makeAdapter());
    const tools = createLoopTools(coordinator, makeSessionClient());

    const result = await (tools.loop_start as any).execute({}, toolContext());

    expect(typeof result).toBe("string");
    expect(result).toContain("prompt");
    // The invalid call must not register a loop.
    expect(coordinator.getLoopState(toolContext().sessionID)).toBeUndefined();
  });

  it("loop_start with prompt only applies schema defaults (iterations=5, mode=inherit)", async () => {
    const coordinator = new LoopCoordinator(makeAdapter());
    const tools = createLoopTools(coordinator, makeSessionClient());

    const result = await (tools.loop_start as any).execute(
      { prompt: "do the thing" },
      toolContext(),
    );
    ...
    const state = coordinator.getLoopState(toolContext().sessionID);
