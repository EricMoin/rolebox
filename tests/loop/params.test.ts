...
describe("parseLoopParams", () => {
  ...
  it("returns defaults when no args provided", () => {
    ...
    if (result.ok) {
      ...
      expect(result.value.mode).toBe("inherit");
      expect(result.value.clamped).toBeUndefined();
  ...
  it("parses positional with only mode", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.iterations).toBe(5); // default
      expect(result.value.mode).toBe("fresh");
    }
  });
  ...
  it("parses key-value: iterations=7 only", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.iterations).toBe(7);
      expect(result.value.mode).toBe("inherit");
    }
  });
  ...
  it("rejects iterations=0", () => {
    ...
    if (!result.ok) {
      expect(result.error).toBe("iterations must be >= 1");
    }
  });
  ...
  it("clamps iterations=99 to 50 with clamped=true", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.iterations).toBe(50);
  ...
  it("unknown mode (e.g. mode=xyz) → inherit + warning", () => {
    ...
    if (result.ok) {
      expect(result.value.mode).toBe("inherit");
      expect(result.value.warning).toMatch(/unknown.*mode/i);
    }
  });
  ...
  it("non-numeric iterations string uses default", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.iterations).toBe(5); // DEFAULT_ITERATIONS
    }
  });
