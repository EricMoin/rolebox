...
describe("readRuntimeState — deliberate exclusions", () => {
  it("ignores the dsh activerole- sidecar (a platform-specific domain)", () => {
    const dir = makeTempDir();
    try {
      const stateDir = join(dir, ".rolebox", "state");
      writeStateFile(stateDir, "activerole", {
        version: 1,
        sessions: [{ sessionId: "s1", roleId: "role-a", updatedAt: 1 }],
      });

      const state = readRuntimeState(dir);

      // The sidecar is deliberately excluded from every reader section; its
      // observability is served by the dsh routes, not this generic reader.
      expect(state.dispatchTasks).toEqual([]);
      expect(state.functionSessions).toEqual([]);
      expect(state.loops).toEqual([]);
    } finally {
