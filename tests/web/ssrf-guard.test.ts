...
import type { Result } from "../../src/utils/result.ts";

/** Narrow a rejected verdict to its reason — fails loudly if the URL was accepted. */
function rejectionReason(result: Result<void, string>): string {
  if (result.ok) throw new Error("expected the URL to be rejected");
  return result.error;
}
...
describe("validateUrl", () => {
  ...
  it("rejects empty string", () => {
    ...
    expect(rejectionReason(result)).toContain("non-empty string");
  });
  ...
  it("rejects URL with no protocol scheme", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("missing a protocol scheme");
  });
  ...
  it("rejects file:// protocol", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("not allowed");
  });
  ...
  it("rejects malformed URL that cannot be parsed", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("malformed");
  });
  ...
  it("blocks localhost", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("localhost");
    expect(rejectionReason(result)).toContain("internal");
  });
  ...
  it("blocks 10.0.0.0/8 RFC 1918 Class A", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("private");
  });
  ...
  it("blocks 169.254.0.0/16 link-local (lower edge)", () => {
    ...
    expect(result.ok).toBe(false);
    expect(rejectionReason(result)).toContain("link-local");
  });
