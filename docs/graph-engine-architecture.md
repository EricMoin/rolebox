...
| 23 | `condition-resolver.ts` | Default `on_condition` edge resolver: evaluates `signal_observed(<type>)` and `artifact_exists(<name>)` against a source node; unknown conditions are false. | `defaultConditionResolver`, `EdgeConditionResolver` | `condition-resolver.ts:75` (defaultConditionResolver) |
