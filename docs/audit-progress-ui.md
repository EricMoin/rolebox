...
- `install.ts` — `const extractedDir = await downloadRole(...)` runs the whole download (no output inside).
- `install.ts` — `console.log(\`✓ Installed ${parsed.roleId}@${version} from ${registryName}\`)` — printed only on success.
- `install.ts` — `console.log(\`Run \`rolebox sync opencode\` to deploy\`)` — the follow-up hint.

There is **no** "resolving", "downloading", or "extracting" line emitted before/around the work. Before `downloadRole` returns, the user sees nothing (aside from the already-installed short-circuit at `install.ts`).

`src/cli/commands/update.ts` emits per-role start/end lines but **no download-time progress**:

- `update.ts` — `const extractedDir = await downloadRole(...)` — again silent.
- `update.ts` — `console.log(\`✓ Updated ${entry.role} from ${entry.version} to ${latestVersion}\`)` — printed only after the full pipeline completes.
- Warnings on failure at `update.ts`.
...
`src/cli/registry-client.ts`, `downloadRole` performs the download as one uninterruptible, non-streamed, non-instrumented sequence:

- `registry-client.ts` — `response = await fetch(url, { headers, redirect: "follow" })` — the tarball fetch resolves only when the **entire** body is buffered (no stream consumption).
- `registry-client.ts` — `const buffer = Buffer.from(await response.arrayBuffer())` — the body is fully materialized into memory; **zero incremental reads**, so no byte-progress can be observed.
- `registry-client.ts` — `writeFileSync(archivePath, buffer)` — synchronous disk write of the whole buffer.
- `registry-client.ts` — `runSpawnSync("tar", ["--version"], { stdio: "ignore" })` — tar availability probe (silent).
- `registry-client.ts` — `runSpawn("tar", ["xzf", archivePath, "--strip-components=1", "-C", extractDir], { stdio: "inherit" })` — extraction streams directly to the user's **stdout/stderr** (because of `stdio: "inherit"`), which can actually emit tar's own verbose output in some tar variants — but with no `-v` flag tar is silent by default. So extraction is also silent in practice.
...
`format.ts` — ANSI color/wrap helpers:
...
`format.ts` — status symbols and phase glyphs:
...
`format.ts` — `bar(current, total, width=10)`:
...
- `display-helpers.ts` — `formatDuration(ms)` — human-readable duration (for elapsed-time and ETA display).
- `display-helpers.ts` — `compactDuration(ms)` — single-unit compact duration.
- `display-helpers.ts` — `truncate(s, maxLen)` — ellipsis truncation for long asset names.
- `display-helpers.ts` — **`barSegments(current, total, width=6)`** — returns `{ filled, empty }`, the numeric segment model behind a bar (composable with `format.ts` colors).
- `display-helpers.ts` — **`statusGlyph(status)`** — canonical glyph per status (`running ▸`, `completed ✓`, `error ✗`, `pending ●`, `cancelled ⊘`, `timeout ⏱`). Reusable for phase status.

Verbatim from `src/utils/display-helpers.ts`:

`display-helpers.ts` — `formatDuration(ms)`:
...
`display-helpers.ts` — `barSegments(current, total, width=6)`:
...
`display-helpers.ts` — `statusGlyph(status)`:
...
Confirmed in `package.json` (`"@clack/prompts": "^0.9.0"`) and used in:

- `src/cli/commands/init/init-prompts.ts` — `import * as clack from '@clack/prompts'`
- `src/cli/commands/config.ts` — `import * as clack from "@clack/prompts"`
- `src/cli/commands/checkpoint/checkpoint-clean.ts` — `import * as clack from "@clack/prompts"`
...
- **No `--quiet` / `-q`** on either command — there is no way to suppress progress output. `install.ts` and `update.ts` declare only positional `role` (and `--no-cache` on update).
...
- **`CI`** — not checked in `install.ts` / `update.ts` (only in `postinstall.ts`).
...
- `src/cli/postinstall.ts`:
  ...
  - `if (process.env["ROLEBOX_NO_WELCOME"]) return;` — an existing `ROLEBOX_*` opt-out env var.
- `src/cli/commands/init/init-prompts.ts`:
  - `if (!process.stdin.isTTY) { throw new Error('Interactive prompts require a TTY...') }` — stdin-TTY guard for interactive UI.

Verbatim from `src/cli/postinstall.ts` (`showWelcome`, the TTY/CI/opt-out guards):
...
Verbatim from `src/cli/commands/init/init-prompts.ts` (`runInteractiveWizard`, the stdin-TTY guard):
...
- **done** — the existing success line (`install.ts` / `update.ts`), kept for compatibility.
...
To observe byte progress, `downloadRole` must consume the body incrementally instead of `await response.arrayBuffer()` (`registry-client.ts`). Sketch:
...
The renderer is injected as an optional callback (or an options param) so `downloadRole` stays testable and the current silent default is preserved for non-interactive callers. The tar child should be switched from `stdio: "inherit"` (`registry-client.ts`) to `stdio: "pipe"` so its output can be relayed through the phase renderer (and not corrupt the progress line).
...
- **reason** — reuse the existing error messages from `registry-client.ts` (`network error downloading role`, `failed to download role … HTTP 403/404`, `extraction failed … tar exited with code …`).
- **retry N/M** — a bounded retry counter (default, e.g. 3 attempts) around `downloadRole`; the retry-aware loop lives in the command handlers (`install.ts` / `update.ts`) or inside `downloadRole` via an options param. On final failure, rethrow the last error so the existing `update.ts` catch still surfaces `Warning: failed to update …`.
