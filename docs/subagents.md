...
Pass `session_id` (the task ID from a previous dispatch) to re-prompt a subagent in the same subagent session. This preserves the conversation history, so the subagent picks up where it left off.
...
Subagents can have their own `skills/` and `functions/` directories (file-based declaration only). On opencode, skills from subagents are symlinked into the global skills dir as `rolebox--{parentId}--{childId}--{skillName}`.
